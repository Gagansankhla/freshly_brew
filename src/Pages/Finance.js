...
export default function FinanceNews() {
  const [financeNewsList, setFinanceNewsList] = useState([]);

  useEffect(() => {
    axios.get('https://newsapi.org/v2/top-headlines?country=us&category=business&apiKey=637111f0eaa24dc69289b969cfd0dcd3')
      .then(response => {
        setFinanceNewsList(response.data.articles || []);
      })
      .catch(error => {
        console.error('Error fetching finance news articles:', error);
      });
  }, []);

  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap' }}>
      {financeNewsList.map(article => (
